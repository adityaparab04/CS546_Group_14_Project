...
const convertObject = (id) => {
    ...
    catch (e){ throw "Id not supported"
    }
}
//Checked
async  function createUser(usernameLower,age,password,emailLower) {
    ...
    if (age == null || age == undefined || age.length == 0) throw "Invalid age parameters"
...
async function deleteUser(_id) {
    ...
    var newId = convertObject(_id);
    const deletionInfo = await userCollection.removeOne({ _id: newId });
    if (deletionInfo.deletedCount === 0) {
        throw `Could not delete user with id of ${newId.toString()}`;
    }
